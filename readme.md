...
**Analysis → Thermal hotspots** maps regional kinetic energy over a selected frame window. Supply velocities with explicit mass/velocity units, or stored kinetic energy with its units and reference. Choose a stationary device, center-of-mass removal for selected atoms inside the grid, or per-bin center-of-mass removal. The map is an atom-exposure average (integrated energy divided by integrated population), not a persistence statistic. Recorded timestamps determine trapezoidal weights when available; otherwise MD steps do. Empty and undersampled bins are transparent. Kelvin requires suitable translational degrees of freedom; stored energies require an explicit post-reference value.

Large fixed-topology TorchSim HDF5 trajectories use a sampled preview, bounded numeric atom batches, and cancellable reduction in the file-owning worker. Cell-following and fixed-device grids support triclinic cells and per-axis periodicity. Slices preserve cell angles and never interpolate through empty bins. The standard atom-and-bond viewer remains the default at every system size. A completed hotspot analysis uses a spatially colored point view while its pane is open and when opening export or camera-flight controls from that view. Threshold and display changes reuse one completed map; analysis changes require recalculation. `run.compute_hotspots(options)` exposes the calculation programmatically when the source provides numeric atom access. The default 128 MiB budget reserves analysis buffers, a previous result, and slice buffers; it is not a total browser/GPU memory limit. Storage chunks above 8 MiB are rejected and must be rechunked. Large text/ASE and Reference MD readers do not yet provide this bounded numeric path.
