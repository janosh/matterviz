...
// Requires: npm run dev (server on port 3000), brew install webp (for cwebp)
...
const pages: PageConfig[] = [
  { url: `/`, name: `landing-page`, wait: 3000, scroll: 100 },
  {
    ...
    actions: [
      [`click`, `input[placeholder="Select a heatmap"]`],
      [`click`, `ul.options li:has-text("Atomic Mass")`],
    ],
    wait: 1500,
  },
...
for (const { url, name, actions, wait, scroll = 200 } of pages) {
  ...
  try {
    ...
    console.warn(`  cwebp failed, keeping PNG: ${err instanceof Error ? err.message : err}`)
