...
const flatten_value = (value: unknown): unknown => {
  if (Array.isArray(value)) {
    // Compact small primitive arrays (e.g. Miller indices) into `(1,1,1)` so
    // they render as sortable/groupable table cells.
    const is_primitive = value.every(
      (item) => typeof item === `number` || typeof item === `string`,
    )
    return value.length <= 6 && is_primitive ? `(${value.join(`,`)})` : DEEP_OBJECT_PLACEHOLDER
  }
  // Only objects nested deeper than max_depth reach here; collapse them.
  if (value && typeof value === `object`) return DEEP_OBJECT_PLACEHOLDER
  return value
}
