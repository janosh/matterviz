<script lang="ts">
  ...
  let {
    ...
    suggested_stride,
    setup_error,
  } = $derived(analysis_pane_setup(trajectory, suggest_vacf_frame_stride, frame_stride))
...
>
  ...
  {:else}
    ...
    {:else if is_lazy}
      <StatusMessage
        type="warning"
        message="Indexed trajectory: {loaded_frames} of {total_frames} frames are in memory. VACF streams the full payload{loader_data_available
          ? ``
    ...
    <div class="analysis-controls vacf-controls">
      <label>
        Max lag
