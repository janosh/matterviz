<script lang="ts">
  ...
  let {
    ...
    comparison_options?: RouteComparisonOptions
  } = $props()

  function cell_preview(text: string, lines: number) {
    ...
    return {
      ...
      measure(node: HTMLElement) {
        node.style.setProperty(`--max-cell-lines`, String(lines))
        const update = () => {
  ...
  const max_cell_lines = $derived(comparison_options.max_cell_lines ?? 8)
...
{#snippet cell(text: string, label: string)}
  {@const preview = cell_preview(text, max_cell_lines)}
  {#if preview.clipped}
    <Popover
      trigger_mode="hover"
      trap_focus={false}
      ...
      align="start"
      aria-label={label}
      style="white-space: pre-line; max-height: min(60dvh, 30rem); overflow-y: auto"
    >
      {#snippet trigger(props)}
        <button
          ...
          {...props}
          {@attach preview.measure}>{preview.text}</button
        >
      {/snippet}
      {preview.text}
    </Popover>
  {:else}
    <span class="cell-preview" {@attach preview.measure}>{preview.text}</span>
  {/if}
{/snippet}
        ...
        <tbody>
          <tr
            ><th scope="row">Weighted score</th>{#each compared as route (route.id)}<td
                ...
                <p>{@render cell(tradeoff(route), `score tradeoff`)}</p></td
              >{/each}</tr
          >
          {#each metrics as { label, hint, value } (label)}
            <tr
              ...
              >
              {#each compared as route (route.id)}<td>{@render cell(value(route), label)}</td
                >{/each}
            </tr>
          {/each}
        </tbody>
...
<style>
  ...
  .cell-preview {
    ...
    -webkit-line-clamp: var(--max-cell-lines);
    line-clamp: var(--max-cell-lines);
    overflow: hidden;
    width: 100%;
  }
  button.cell-preview {
