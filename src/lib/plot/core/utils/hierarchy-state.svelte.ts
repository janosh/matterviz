...
> {
  ...
  // Roving tabindex: the last-focused node while still rendered, else `first_visible`.
  // Without it, tabbing a large chart would visit every arc/cell.
...
// zoom_root_id is a plain prop, never derived from the layout it feeds - the arcs depend on
// it, so reading it back off them would close a cycle. Shared by Treemap and Sunburst, and
// read only while bucketing measures against the view root: a zoom would otherwise rebuild
// the layout (re-measuring every label) for an identical result.
