...
// Legend muting + hover dimming for one node. The hovered node + its ancestors/descendants
// stay fully opaque, other nodes dim to 0.3; muted categories dim hardest. Pre-order indexing
// makes the ancestor/descendant tests O(1): a subtree is the contiguous index range
// [node_idx, subtree_end]. Returns an accessor rather than an array: only the nodes currently
// on screen are ever read, where materializing one object per node in the whole hierarchy
// allocated the entire tree on every hover move.
