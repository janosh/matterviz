<script lang="ts">
  ...
  // A lone series uses the configured bar color; with several, each gets its own (`color`,
  // then the cycled palette). Keyed on `series`, not the visible subset the legend outlives:
  // gating on that painted every swatch `bar.color` once all but one series were hidden.
