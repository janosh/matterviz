...
// Legend rows in render order. A null series keeps its index (points and styles are looked up
// by it) but contributes no row, else the legend shows a phantom `Series N`. The rendered
// legend and the decoration solver's footprint tracker both build from here.
...
export function build_legend_data<Metadata = Record<string, unknown>>(
  ...
  const display_style_for = (
    ...
    if (!series_markers.includes(`points`)) {
      ...
      // Fall back to stroke when the fill is missing/none/transparent. Reads the alpha
      // channel: a `startsWith('rgba(')` test called every rgba() color transparent.
