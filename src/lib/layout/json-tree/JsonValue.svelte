<script lang="ts">
  ...
  // Teardown must be unmount-only: an effect cleanup also runs per re-run, and ctx.settings
  // is a fresh object per settings change, so a toggle mid-flash cancelled the un-flash timer.
