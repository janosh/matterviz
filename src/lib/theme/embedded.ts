...
export function watch_theme(
  ...
  // Unlike watch_dark_mode, this one does notify on subscribe: its callers have no separate
  // way to read the host theme, so the first value has to arrive through the callback.
  notify()
  return () => {
    clearTimeout(timer)
    stop_observing()
