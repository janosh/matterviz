...
export const spectroscopy_stream_channels = (
  ...
  const declared = (key: string | null): TrajectoryRunSignal | undefined =>
    key ? run.signals?.[key] : undefined
  const velocity = declared(velocity_key)
  const velocity_channel: `vector` | `signal` | null = !velocity_key
    ? null
    : velocity === undefined
      ? has_site_velocities(run, velocity_key)
        ? `vector`
        : null
      : is_loaded_signal(velocity)
        ? null
        : streams_as_vector(run, velocity)
          ? `vector`
          : `signal`
  const response_keys = [infrared_key, raman_key].filter((key): key is string => {
    const signal = declared(key)
    return Boolean(key) && (signal === undefined || is_signal_descriptor(signal))
  })
  return {
    vector_keys: velocity_key && velocity_channel === `vector` ? [velocity_key] : [],
    signal_keys: [
      ...(velocity_key && velocity_channel === `signal` ? [velocity_key] : []),
      ...response_keys,
    ],
  }
}
