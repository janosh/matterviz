...
export function fit_eos(
  ...
  for (let iter = 0; iter < MAX_ITERATIONS && !converged; iter++) {
    ...
    // Restart the ladder each outer iteration, as a textbook LM does: a carried-over damping
    // of 1e11 froze every later step at ~1e-11 of Gauss-Newton and was read as convergence
    ...
    // tiny step, or no downhill direction left; with the reset above, exhausting the ladder
    // means |Jᵀr| really is stationary (1e-12 to 1e-17 of the energy norm on every fixture)
    converged = !moved
  }
