...
// Convert flat array to 3D grid [nx][ny][nz]
    ...
    for (let iy = 0; iy < ny; iy++) {
      for (let iz = 0; iz < nz; iz++) {
        ...
        if (is_complete || flat_idx < flat_data.length) {
          grid[ix][iy][iz] = flat_data[flat_idx] / divisor
        }
      }
    }
  }
  ...
  try {
    ...
    for (let elem_idx = 0; elem_idx < element_symbols.length; elem_idx++) {
      ...
      for (let count_idx = 0; count_idx < count; count_idx++) {
        const coords = lines[line_idx + atom_idx + count_idx]
          .trim()
          .split(/\s+/)
          .slice(0, 3)
          .map(Number) as Vec3
    ...
    for (let vol_idx = 0; vol_idx < 2; vol_idx++) {
      ...
      // Guard against degenerate lattices with zero volume
      const divisor = Math.abs(lattice_params.volume) > 1e-30 ? lattice_params.volume : 1
      const grid = flat_to_3d_grid(flat_data, ngx, ngy, ngz, divisor)
    ...
    console.error(`Error parsing CHGCAR file:`, error)
    return null
  }
}
...
export function parse_cube(content: string): VolumetricFileData | null {
  try {
    ...
    const raw_n_atoms = header[0]
    const n_atoms = Math.abs(raw_n_atoms)
    const has_orbital_header = raw_n_atoms < 0
    ...
    const grid = flat_to_3d_grid(flat_data, n_grid[0], n_grid[1], n_grid[2])
