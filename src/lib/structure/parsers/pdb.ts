...
export const parse_pdb = (content: string): AnyStructure => {
  ...
  if (has_further_entry) {
    console.warn(`PDB contains multiple END-terminated entries; parsed the first`)
  }
  ...
  for (const line of lines) {
    ...
    if (record === `CRYST1` && cell_params === null) {
      const params = read_cryst1_params(line)
      if (params === null)
        throw new Error(`PDB CRYST1 record has invalid cell parameters: '${line.trim()}'`)
  ...
  if (atom_lines.length === 0) throw new Error(`No ATOM or HETATM records found in PDB file`)
  if (model_count > 1) {
    console.warn(
  ...
  if (skipped_alt_locs > 0) {
    console.warn(`PDB: skipped ${skipped_alt_locs} atom(s) with alternate location indicators`)
  }
