...
// and element data is resolved once per element, not once per site (a 10k-atom MD frame has a
// handful of elements and this runs every frame). `unknown_label` stands in for unresolvable.
...
// The ternaries never yield -0, so a reversed shift still keys and compares as its forward
// twin (`-0` and `0` stringify differently)
