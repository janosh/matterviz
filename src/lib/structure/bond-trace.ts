...
// detail is a thunk because most call sites serialize bond lists to build it. Passing the object
// directly evaluated those JSON.stringify calls on every bond emit even with tracing off, which
// slowed every e2e shard from ~5 to 17+ minutes.
export const bond_trace = (
  event: string,
  id: string,
  ...
  if (Array.isArray(sink)) {
    sink.push({ ...detail(), event, id, t: Math.round(performance.now()) })
  }
}
