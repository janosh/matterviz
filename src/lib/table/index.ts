...
export const merge_domains = (domains: [number, number][]): [number, number] | null =>
  domains.length === 0
    ? null
    : [Math.min(...domains.map(([lo]) => lo)), Math.max(...domains.map(([, hi]) => hi))]
...
export function make_cell_color_scale(
  ...
  const smallest_positive = min(numeric_vals)
  if (scale_type === `log` && range[0] <= 0 && smallest_positive != null && range[1] > 0) {
    range[0] = smallest_positive
  }
