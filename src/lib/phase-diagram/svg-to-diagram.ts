...
// gap between one segment's end and the next one's start begins a new subpath; each ring is
// closed the way SVG closes a subpath when filling it.
...
// Outline rings of a filled shape plus their bounding box, or null if it encloses no area
function shape_outline(el: Element): { bbox: Vec4; rings: Vec2[][] } | null {
  const rings = shape_rings(el) ?? []
  const verts = rings.flat()
  if (verts.length < 3) return null // fewer than 3 corners encloses no area
  const [min_x, max_x] = array_extent(verts.map(([x_px]) => x_px))
  const [min_y, max_y] = array_extent(verts.map(([, y_px]) => y_px))
  return { bbox: [min_x, min_y, max_x, max_y], rings }
}

// Collect all non-background filled shapes (candidate phase region fills). Transformed shapes
// are skipped since shape_outline ignores transforms (their pixel coords would be wrong).
function extract_filled_shapes(doc: Document): FilledShape[] {
  ...
  for (const el of Array.from(doc.querySelectorAll(`rect, polygon, path, use`))) {
    ...
    const outline = shape_outline(el)
    if (outline) shapes.push({ fill, ...outline })
  }
  return shapes
}

// Fill of the smallest filled shape actually containing the given pixel point. The bounding
// box is only the cheap reject: a concave phase field's bbox covers its neighbours, so
// deciding on the bbox alone painted a field with a neighbour's colour.
