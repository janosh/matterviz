<script lang="ts">
  ...
  // color to the top) and the values themselves for the robust quantiles. Only the filtered-in
  // rows/columns count, so filtering rescales the colors, as HeatmapTable's column_stats does.
  ...
  // Label tracks size to their content, so filtering moves where the grid starts without
  // resizing the container the observer above watches.
  $effect(() => {
