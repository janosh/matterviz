...
<style>
  /* Flat rules on purpose: happy-dom (vitest) drops any rule that contains nested blocks,
     and the Structure tests assert this menu's layering via getComputedStyle. */
  .view-mode-control {
    display: flex;
    position: relative;
    align-items: center;
  ...
  .view-mode-dropdown > :global(.view-mode-option:first-child) {
    border-top-left-radius: 3px;
    border-top-right-radius: 3px;
  }
  .view-mode-dropdown > :global(.view-mode-option.selected) {
    color: var(
      --accent-color,
      var(--menu-option-selected-color, light-dark(#2563eb, #6ea8ff))
    );
  }
  .view-mode-dropdown > :global(.view-mode-option span) {
    font-weight: 500;
    white-space: nowrap;
    overflow: hidden;
