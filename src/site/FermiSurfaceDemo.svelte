<script lang="ts">
  ...
  onMount(() => {
    const requested = sync_url ? file_param() : null
    const file_from_url = requested
      ? fermi_surface_files.find((file) => file.name === requested)
      : null
...
<FermiSurface
  ...
  on_file_load={({ source_filename }) => {
    ...
    if (sync_url) set_file_param(source_filename)
  }}
  tooltip_config={{
