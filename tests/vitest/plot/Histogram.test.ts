...
describe(`Histogram`, () => {
  ...
  test.each<[string, HistogramSeriesInput, HistogramSeries]>([
    ...
    expect(series_data).toStrictEqual(expected)
    expect(Object.keys(series_data)).not.toContain(`x`)
    expect(Object.keys(series_data)).not.toContain(`y`)
    expect(Object.keys(series_data)).not.toContain(`line_style`)
