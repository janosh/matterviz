...
const measured = (
  ...
  candidate: create_tick_candidate({ id, strategy, rotation_deg, labels: [id] }),
  measurements: {
    collisions: 0,
    edge_overflow_px: 0,
    band_fraction: 0,
...
test(`uses the fixed score across feasible candidates`, () => {
  const upright = measured(`upright`)
  const rotated = measured(`rotated`, {}, `rotate`, 90)

  expect(select_tick_candidate([rotated, upright])).toBe(upright)
})
