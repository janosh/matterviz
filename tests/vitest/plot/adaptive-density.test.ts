...
describe(`adaptive density utilities`, () => {
  ...
  ] satisfies DensePointSeries<{ id: string }>[]
  ...
  it(`uses exact density-bin assignment for boundary points`, () => {
    ...
    expect(point_in(0)).toBeNull()
    expect(point_in(1)?.point_id).toBe(`boundary`)
  })
  ...
  it.each([
    ...
    for (const range of [
      ...
      for (const axis of [`x`, `y`] as const) {
        ...
        const binned = bin_points(series, x_range, y_range, 4, 4, undefined, order)
        expect(binned).toEqual(bin_points(series, x_range, y_range, 4, 4))
        const scale = (value: number) => value * 10
        const points = [...visible_points(series, x_range, y_range, scale, scale, order)]
        expect(points).toEqual([...visible_points(series, x_range, y_range, scale, scale)])
        expect(binned.visible_count).toBe(points.length)
        if (range.some(Number.isNaN)) expect(points).toEqual([])
      }
    }
  })
})
