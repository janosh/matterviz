...
describe(`BinnedScatterPlot`, () => {
  ...
  test(`pulse ticks repaint only the marked-points overlay`, async () => {
    ...
    // happy-dom 20.13 stopped defining per-property accessors on CSSStyleDeclaration.prototype,
    // so a `width` setter spy throws. `style.width = x` routes through setProperty, which is
    // the CSSOM API either way.
