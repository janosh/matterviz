// Regression tripwires for the hot paths hand-timed in #438. Each subsystem gets one seeded
// synthetic generator (nothing on disk), a median-of-N timing and a stored baseline; a run
// fails only above 2x that baseline, after scaling by how much slower than the baseline
// machine this one proves on a fixed reference workload. That keeps 2x meaningful on a
// throttled CI runner without loosening it into a band no real regression would ever hit.
// Opt in locally with MATTERVIZ_PERF=1; CI runs it in its own job (see test.yml).
...
const BASELINES = {
  ...
  'e_above_hull 775 ternary entries': 3.4,
...
const enabled = process.env.MATTERVIZ_PERF === `1`
// 5 reps of a ~1 s mount plus cleanup can exceed vitest's 5 s default on a loaded runner
describe.skipIf(!enabled)(`perf baselines`, { timeout: 120_000 }, () => {
  beforeAll(async () => {
  ...
  test(`e_above_hull 775 ternary entries`, async () => {
    const entries = make_entries(775)
    await measure(`e_above_hull 775 ternary entries`, () => {
      const distances = calculate_e_above_hull(entries, entries)
      expect(Object.keys(distances)).toHaveLength(775)
