...
describe(`@xrd/ api and compute_xrd_pattern options`, () => {
  test(`unknown radiation key throws`, () => {
    ...
    expect(() => compute_xrd_pattern(structure, { wavelength: `FooBar` as never })).toThrow(
      /Unknown radiation key/i,
    )
  })
  ...
  test.each([{ t_range: [0, 30] as Vec2 }, { t_range: [10, 60] as Vec2 }])(
    `two_theta_range bounds respected and x sorted`,
    ({ t_range }) => {
      const structure = make_simple_cubic(2)
