...
describe(`compute_xrd_pattern edge cases`, () => {
  ...
  // gives h_max 246 but 6.1e7 points (~8.5 GB), so electrons default to [0, 5]° instead
...
describe(`electron_wavelength`, () => {
  ...
  test.each([
    ...
    // rules out the non-relativistic h/sqrt(2·m0·e·V), which is 9.3% long (0.02741 at 200 kV)
...
describe(`compute_xrd_pattern options`, () => {
  ...
  // 2.14e-27 — relative scaling used to emit that as a forbidden (100) at 21.9465°, y = 100
  ...
  // D has no ELEM_SYMBOLS entry but shares H's electronic structure (and its
  // atomic_scattering_params row); the Z lookup used to reject it, even for Z-free electrons
  test.each([`xray`, `electron`] as const)(
    `deuterium diffracts as hydrogen for %s`,
    (radiation) => {
      const sites = (element: string) => [
        { element, abc: [0, 0, 0] as Vec3, label: element },
        { element: `O`, abc: [0.5, 0.5, 0.5] as Vec3, label: `O` },
      ]
      const options =
        radiation === `xray`
          ? ({ wavelength: `CuKa` } as const)
          : ({ radiation, accelerating_voltage: 200 } as const)
      const deuterated = compute_xrd_pattern(make_crystal(3, sites(`D`)), options)
      const protonated = compute_xrd_pattern(make_crystal(3, sites(`H`)), options)
      expect(deuterated.x).toEqual(protonated.x)
      expect(deuterated.y).toEqual(protonated.y)
    },
  )
