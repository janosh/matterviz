...
describe(`grid_data_range`, () => {
  test.each([
    ...
    { grid: [[[-10, 1]]], min: -10, max: 1, abs_max: 10, label: `abs_max driven by min` },
    { grid: [[[0, 0], [0, 0]]], min: 0, max: 0, abs_max: 0, label: `uniform zero` },
    { grid: [[[42]]], min: 42, max: 42, abs_max: 42, label: `single element` },
    { grid: [[[-3.5]]], min: -3.5, max: -3.5, abs_max: 3.5, label: `single negative` },
  ])(`$label: min=$min max=$max abs_max=$abs_max`, ({ grid, min, max, abs_max }) => {
...
describe(`auto_isosurface_settings`, () => {
  test(`sets isovalue to 20% of abs_max for positive data`, () => {
    const settings = auto_isosurface_settings([[[0, 5, 10]]])
    expect(settings.isovalue).toBeCloseTo(10 * 0.2)
    expect(settings.show_negative).toBe(false)
  })

  test(`enables show_negative for data with significant negative values`, () => {
    const settings = auto_isosurface_settings([[[-5, 0, 10]]])
    expect(settings.isovalue).toBeCloseTo(10 * 0.2)
    expect(settings.show_negative).toBe(true)
  })

  test(`falls back to default isovalue for all-zero grid`, () => {
    const settings = auto_isosurface_settings([[[0, 0], [0, 0]]])
    expect(settings.isovalue).toBe(DEFAULT_ISOSURFACE_SETTINGS.isovalue)
    expect(settings.show_negative).toBe(false)
  })

  test(`preserves default opacity, colors, and wireframe`, () => {
    const settings = auto_isosurface_settings([[[1, 2, 3]]])
