...
describe(`Isosurface`, () => {
  ...
  test.each([
    [`on_error prop`, true, false, `prop`],
    [`context handler`, false, true, `context`],
    [`prop over context`, true, true, `prop`],
  ])(
    `worker failure after construction reports through %s`,
    async (_label, with_prop, with_context, expected) => {
      ...
      const [winner, loser] =
        expected === `prop` ? [prop_handler, context_handler] : [context_handler, prop_handler]
