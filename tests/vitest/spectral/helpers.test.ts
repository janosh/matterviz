...
describe(`normalize_densities`, () => {
  ...
  it(`integral mode makes ∫ = 1 on a non-uniform grid`, () => {
    const grid = [0, 0.1, 0.2, 3, 10]
    const trapezoid = (ys: number[]) =>
      grid
        .slice(1)
        .reduce(
          (acc, x_val, idx) => acc + ((ys[idx] + ys[idx + 1]) / 2) * (x_val - grid[idx]),
          0,
        )
    for (const dens of [densities, [1, 1, 1, 1, 1]]) {
      expect(trapezoid(normalize_densities(dens, grid, `integral`))).toBeCloseTo(1, 12)
    }
  })
...
describe(`apply_gaussian_smearing`, () => {
  ...
  // the old measure-free form weighted by POINT DENSITY: on this grid a flat DOS came out
  // anywhere from 0.235 to 1.950 over the interior, i.e. 95% off. gaussian_kernel_smooth is
  // its Nadaraya-Watson sibling: exact on constants everywhere, with no quadrature measure.
...
describe(`compute_frequency_range`, () => {
  ...
  it.each([
    ...
    // imaginary-mode clamp: this case reported [-0.3102, 15.3002]
    [
