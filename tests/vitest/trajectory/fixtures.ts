...
export const h5_bytes = async (
  ...
  try {
    ...
    file.close()
    file_closed = true
    return Uint8Array.from(file_system.readFile(temp_filename)).buffer
  } finally {
    if (!file_closed) {
      try {
        file.close()
    ...
    try {
      file_system.unlink(temp_filename)
    } catch {
...
export const make_h5_buffer = (datasets: H5Spec[]): Promise<ArrayBuffer> =>
  h5_bytes(`torn-tail`, (file) => {
    for (const [name, data, shape] of datasets) create_dataset(file, name, data, shape)
  })
...
export const make_grouped_h5_buffer = (
  ...
  h5_bytes(`grouped`, (file) => {
    for (const [group_idx, { name, atomic_number, x_position }] of groups.entries()) {
      ...
      create_dataset(group, `positions`, [x_position, 0, 0], [1, 1, 3])
      create_dataset(group, `atomic_numbers`, [atomic_number], [1])
    ...
    if (include_unrelated_data_signals) {
      ...
      create_dataset(data, `masses`, [2], [1])
      create_dataset(data, `dipole`, [1, 0, 0, 0, 1, 0], [2, 3])
      create_dataset(file.create_group(`steps`), `dipole`, [0, 1], [2])
    }
  })
...
export const make_torch_sim_signal_buffer = ({
  ...
  h5_bytes(`torch-sim-signals`, (file) => {
    ...
    create_dataset(steps, `positions`, [0, 1, 2, 3], [4])
    create_dataset(steps, `velocities`, velocity_steps, [velocity_steps.length])
    if (include_dipole_steps) {
      create_dataset(steps, `dipole`, dipole_steps, [2])
    }
    create_dataset(steps, `polarizability`, [0, 2, 4], [3])
...
export const make_reference_md_h5_buffer = (
  ...
  h5_bytes(`reference-md`, (file) => {
    ...
    create_dataset(file.create_group(`replicas`), `global_ids`, global_ids, [
      global_ids.length,
    ])

    const molecule = file.create_group(`molecules`).create_group(`h2o`)
    const topology = molecule.create_group(`topology`)
    create_dataset(topology, `atomic_numbers`, [1, 8], [2])
    create_dataset(topology, `masses_amu`, [1.008, 15.999], [2])
    create_dataset(topology, `pbc`, [0, 0, 0], [3])
    create_dataset(molecule.create_group(`replicas`), `member_seeds`, [7, 8], [2])
    ...
    create_dataset(initial_state, `momenta_sqrt_ev_amu`, Array(12).fill(0), [2, 2, 3])
    create_dataset(
      initial_state,
      `cells_angstrom`,
