...
describe(`gas-thermodynamics: apply_gas_corrections`, () => {
  ...
  // Shifting the O reference changes every oxide's formation energy, but the oxides
  // are returned untouched and `e_above_hull_distances` prefers the e_form_per_atom
  // they cached against the old reference. Left in place, the whole gas correction
  // is a no-op on the hull: pressure and temperature move nothing on screen.
  test(`a shifted reference clears formation energies cached against the old one`, () => {
    ...
    expect(oxide.e_form_per_atom).toBeUndefined()
    expect(oxide.e_above_hull).toBeUndefined()
    expect(oxide.is_stable).toBeUndefined()
