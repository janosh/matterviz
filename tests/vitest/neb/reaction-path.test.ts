...
describe(`path_spline on structures`, () => {
  ...
  test(`the fitted saddle does not depend on which x-axis is plotted`, () => {
    ...
    // Both ways the tangents used to come out wrong on the bead-number axis: rescaling each
    // knot slope by a central-difference ds/di collapsed the two arc widths a knot sits
    // between into their average, and leaving the slopes in eV/Å ignored the arc entirely.
