...
test(`Scene reuses bond colors only for an explicit matching topology and appearance`, async () => {
...
test(`prepared bonds reuse placements and uniform colors across topology and appearance changes`, async () => {
  const structure = {
...
test(`property-colored Scene frames refresh reused atoms and restore element colors`, async () => {
  ...
  try {
    ...
    if (!atoms?.colors) throw new Error(`Expected an instanced atom mesh`)
