...
// vertex set holds all three corners. Nearest-centroid assignment labelled six S-bearing
// domains that own no face of this Y-Ti-O projection.
test(`hull faces are labelled by the domain that owns their vertices`, async () => {
  ...
  for (let face_idx = 0; face_idx < n_faces; face_idx++) {
    const corners = [0, 1, 2].map((vert) => face_idx * 9 + vert * 3)
    const claimants = [...render_pts].filter(([, pts]) =>
      corners.every((base) =>
        pts.some(
          (pt) =>
            Math.abs(pt[0] - pos[base]) < 1e-3 &&
            Math.abs(pt[1] - pos[base + 1]) < 1e-3 &&
            Math.abs(pt[2] - pos[base + 2]) < 1e-3,
        ),
      ),
    )
