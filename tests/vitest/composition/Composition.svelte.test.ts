...
describe(`Composition component`, () => {
  test(`renders with basic props`, () => {
    ...
    expect(doc_query(`.composition`)).toBeInstanceOf(Element)
  })
  ...
  test.each([`pie`, `bubble`, `bar`] as const)(`renders %s mode correctly`, (mode) => {
    ...
    expect(doc_query(`.${mode}-chart`)).toBeInstanceOf(Element)
  })
  ...
  test(`context menu changes propagate to chart components`, async () => {
    ...
    open_context_menu()
    await tick()
    expect(doc_query(`.bubble-chart`)).toBeInstanceOf(Element)
  })
