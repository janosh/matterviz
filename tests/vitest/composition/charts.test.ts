...
describe(`BubbleChart`, () => {
  ...
  // size 0 (an unmeasured container) packs every bubble at r = 0, and the label scale divided
  // by the largest radius: 0 / 0 wrote `font-size: NaNpx` into every tspan
  test(`a collapsed size keeps label font sizes finite`, () => {
    ...
    for (const size of sizes) expect(Number(size.replace(`px`, ``))).not.toBeNaN()
  })
})
