...
// reader keeps guessing (and warning) the way it does without any host
test(`the atom_type_mapping setting reaches the parser and an empty map is omitted`, async () => {
  ...
  const atom_type_mapping = { 1: `Si`, 2: `O` }
  parse_file_content.mockResolvedValueOnce(result(`dump`))
  ...
  update_file(`dump`)
  await vi.waitFor(() => expect(parse_in_worker).toHaveBeenCalledTimes(2))
  expect(parse_in_worker).toHaveBeenLastCalledWith(
    `dump`,
    `dump.json`,
    false,
    expect.objectContaining({
      load_options: { index_above_bytes: 4096, atom_type_mapping },
    }),
  )
})
