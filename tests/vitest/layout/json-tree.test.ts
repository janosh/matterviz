...
describe(`folding`, () => {
  ...
  // The un-flash timer hung off an effect cleanup that re-runs on every ctx.settings change,
  // so a toggle mid-flash cancelled it without rescheduling and the node stayed highlighted
...
describe(`search`, () => {
  ...
  // The search box sits inside the div carrying the tree's keydown handler, so arrow keys
  // typed there yanked focus onto a tree node and Ctrl/Cmd+C copied the selection
