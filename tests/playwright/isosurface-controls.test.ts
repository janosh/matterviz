...
test.describe(`Isosurface page`, () => {
  ...
  test.describe(`Volumetric slices`, () => {
    test(`fills the viewer aspect ratio and overlays vertical chrome`, async ({ page }) => {
      test.setTimeout(IS_CI ? 120_000 : 90_000)
      await select_view_layout(page, `2D cross-section`)
      const slice = page.getByTestId(`volume-slice`)
      ...
      await expect(position_control).toHaveCSS(`opacity`, `0`)
      await page.mouse.move(canvas_box.x + canvas_box.width / 2, canvas_box.y + 20)
      await expect(position_control).toHaveCSS(`opacity`, `1`)
      ...
      const original_slice = await canvas_screenshot(canvas)
      await position_slider.fill(`0.75`)
      await expect(position_slider).toHaveValue(`0.75`)
      await expect_canvas_changed(canvas, original_slice)
      ...
      await expect(page).toHaveURL(/[?&]view=slice(?:&|$)/)
    ...
    test(`switches between HKL, Cartesian, filled, and contour views`, async ({ page }) => {
      ...
      const canvas = slice.locator(`canvas`)
      await wait_for_canvas_rendered(canvas)
      expect(Number(await canvas.getAttribute(`width`))).toBeGreaterThanOrEqual(1024)
