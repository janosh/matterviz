...
const load_page = async (page: Page, frames: number, atoms: number) => {
  ...
  // controls are server-rendered, so wait for hydration (mount_ms is set in onMount) before
  // timing anything; a cold vite dev server can take several seconds to serve the bundle
  await expect
