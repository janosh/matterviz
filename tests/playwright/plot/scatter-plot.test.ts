...
const set_density = async (
  ...
  const set_slider = async (label_text: string, value: number) => {
    const input_locator = section_locator.locator(`label:has-text('${label_text}') input`)
...
const get_colorbar_quadrant = async (section_locator: Locator): Promise<string> => {
...
test.describe(`ScatterPlot Component Tests`, () => {
  ...
  test(`renders basic scatter plot with correct axis labels and ticks`, async ({ page }) => {
    ...
    for (const axis of [`x`, `y`] as const) {
      ...
      // Verify tick values are parseable numbers and in ascending order
      ...
      const spacings = values.slice(1).map((val, idx) => val - values[idx])
      const avg_spacing = spacings.reduce((sum, val) => sum + val, 0) / spacings.length
  ...
  test(`marker sizing scales correctly with data values`, async ({ page }) => {
    ...
    // First three markers of basic_data grow with their data values
    const areas = await Promise.all(
      [0, 1, 2].map(async (idx) => get_bbox_area(await get_marker_bbox(plot_locator, idx))),
    )
  ...
  test(`size_values prop with per-point styling and dynamic configuration`, async ({
    page,
  }) => {
    // Verifies the fix for size_values not working with per-point styling arrays
  ...
  test(`handles color scaling with both linear and log modes`, async ({ page }) => {
    ...
    // Section data includes null/undefined color values, so this also covers
    // graceful null handling during scale transitions
  ...
  test(`legend renders when explicitly configured or for multiple series`, async ({
    page,
  }) => {
  ...
  test(`axis ranges adapt to visible series and preserve on full hide`, async ({ page }) => {
    ...
    // Series A: x=[1,2] y=[3,4], Series B: x=[1,2] y=[1,2]
    // Combined y range spans [1,4] = 3
    const initial_x = await get_tick_range(x_axis)
    const initial_y = await get_tick_range(y_axis)
    ...
    // Hide Series A - ranges should shrink to fit only Series B (y=[1,2])
    await series_a_item.click()
    await expect(series_a_item).toHaveClass(/hidden/)
    ...
    // x range stays the same (both series have x=[1,2])
    expect(after_hide_a_x.range).toBeGreaterThanOrEqual(initial_x.range * 0.95)
    // y range should shrink (only Series B y=[1,2] visible, range=1 vs initial 3)
    expect(after_hide_a_y.range).toBeLessThanOrEqual(initial_y.range)

    // Show Series A again - ranges should expand back to full
    await series_a_item.click()
    await expect(series_a_item).not.toHaveClass(/hidden/)
    await page.waitForTimeout(100)
    ...
    // Ranges should restore to initial (both series visible again)
    ...
    // Hide both series - ranges should NOT snap to [0,1] default
    await series_a_item.click()
    await series_b_item.click()
    await expect(series_a_item).toHaveClass(/hidden/)
    await expect(series_b_item).toHaveClass(/hidden/)
    ...
    // They should remain at the last visible data range
    expect(after_hide_both_x.range).toBeGreaterThan(0)
    expect(after_hide_both_y.range).toBeGreaterThan(0)
  ...
  test(`legend positioning and dragging functionality`, async ({ page }) => {
    ...
    // Wait for smart placement algorithm to calculate position
    // The legend initially renders at fallback position, then moves to calculated position
    // Poll until position stabilizes (not at the initial fallback corner)
    await expect
      .poll(
        async () => {
          const bbox = await legend_locator.boundingBox()
          const plot_box = await plot_locator.boundingBox()
          if (!bbox || !plot_box) return false
          // Check if legend has moved away from fallback top-left position
          // Fallback is ~(plot.x + padding + 10), so check if significantly different
          const min_expected_offset = 50
          return (
            bbox.x > plot_box.x + min_expected_offset ||
            bbox.y > plot_box.y + min_expected_offset
          )
        },
        { timeout: 5000, message: `Waiting for legend to move from fallback position` },
      )
      .toBe(true)
    ...
    const marker_positions = await Promise.all(markers.map((marker) => marker.boundingBox()))
    ...
    expect(
      min_distance,
      `Legend too close to markers: ${min_distance.toFixed(1)}px (threshold: ${distance_threshold.toFixed(
        1,
      )}px, 5% of diagonal)`,
  ...
  test(`control pane functionality and state management`, async ({ page }) => {
    ...
    const { toggle: controls_toggle, pane: control_pane } =
      await open_control_pane(scatter_plot)
  ...
  axis_color_test_cases.forEach(({ plot_id, expected_markers, description }) => {
    test(`${description} renders correctly`, async ({ page }) => {
      ...
      if (
        plot_id.includes(`dual`) ||
        plot_id.includes(`custom`) ||
  ...
  test(`explicit point styling preserved on page load (controls don't override)`, async ({
    page,
  }) => {
  ...
  test(`per-property control touch: only modified control overrides explicit styling`, async ({
    page,
  }) => {
    const plot = page.locator(`#control-precedence-plot.scatter`)
    await expect(plot).toBeVisible()
  ...
  test(`auto-cycling assigns different colors to each series legend item`, async ({
    page,
  }) => {
  ...
  test(`auto-cycling assigns different symbols to each series legend item`, async ({
    page,
  }) => {
    const section = page.locator(`#auto-cycling-test`)
    await expect(section).toBeVisible()
  ...
  test(`tooltip appears and updates when hovering over different markers`, async ({
    page,
  }) => {
