...
describe(`MatterViz Extension`, () => {
  ...
  describe(`Custom Editor File Patterns`, () => {
    ...
    // The selector list is generated by scripts/sync-config.ts, so a format added to
    // $lib/constants reaches the editor registration without touching package.json.
    test(`every shared viewer extension is claimed, bare and gzipped`, () => {
      const shared = [...TEXT_VIEWER_EXTENSIONS, ...BINARY_VIEWER_EXTENSIONS]
      const unclaimed = shared.filter(
        (ext) =>
          !matches_any_pattern(`sample.${ext}`) || !matches_any_pattern(`sample.${ext}.gz`),
      )
