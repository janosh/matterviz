...
def generate_init_stub(stub_root: Path, rust_dir: Path) -> None:
    """Generate top-level __init__.pyi by parsing __init__.py re-exports."""
    py_content = (stub_root / "__init__.py").read_text()
    # Extract `from ferrox._ferrox import (Element, __version__, cell, ...)`
    import_match = re.search(
        r"from ferrox\._ferrox import \(\s*([\s\S]*?)\s*\)", py_content
    )
    if not import_match:
        return
    names = [
    ...
    submodules = [n for n in names if n[0].islower() and not n.startswith("__")]
    lines = [
    ...
    if classes:
        lines.append(f"from ._ferrox import {', '.join(classes)}")
    if submodules:
        lines.append(f"from . import {', '.join(submodules)}")
    # Extra re-exports (e.g. StructureMatcher = structure.StructureMatcher)
    lines.extend(
        f"{m.group(1)} = {m.group(2)}.{m.group(3)}"
        for m in re.finditer(r"^(\w+) = (\w+)\.(\w+)$", py_content, re.MULTILINE)
    )
    lines.append("")
