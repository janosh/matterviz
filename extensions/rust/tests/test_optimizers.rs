...
/// Test Morse dimer converges from various initial bond lengths (stretched and compressed)
#[test]
fn test_morse_dimer_convergence() {
    let d = 4.5;
    let a = 1.9;
    let r_eq = 0.74;
    let config = FireConfig::default();

    // Test both stretched (1.2) and compressed (0.5) starting points
    for (label, r_init) in [("stretched", 1.2), ("compressed", 0.5)] {
        let initial = vec![Vector3::new(0.0, 0.0, 0.0), Vector3::new(r_init, 0.0, 0.0)];
        ...
        assert!(
            ...
            "{label}: should converge, max_force={}",
            max_force(&state)
        );
        ...
        assert!(
            ...
            "{label}: bond length should be ~{r_eq}, got {bond_length}"
        );

        let energy = morse_potential(bond_length, d, a, r_eq);
        assert!(energy < 0.001, "{label}: energy should be ~0, got {energy}");
    }
}
