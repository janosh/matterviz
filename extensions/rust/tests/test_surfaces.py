...
class TestEnumerateMiller:
    ...
    def test_max_1_includes_low_index_planes(self) -> None:
        ...
        indices = surfaces.enumerate_miller(1)
    ...
    def test_excludes_zero(self) -> None:
        ...
        indices = surfaces.enumerate_miller(2)
    ...
    def test_subset_property(self) -> None:
        ...
        for max_idx in range(1, 3):
            indices_lower = set(
                tuple(idx) for idx in surfaces.enumerate_miller(max_idx)
            )
            indices_higher = set(
                tuple(idx) for idx in surfaces.enumerate_miller(max_idx + 1)
            )
...
class TestDSpacing:
    ...
    def test_cubic_d_spacing_formula(self, simple_cubic_structure: dict) -> None:
        ...
        assert surfaces.d_spacing(simple_cubic_structure, 1, 0, 0) == pytest.approx(
            lattice_param, abs=0.01
        )
        assert surfaces.d_spacing(simple_cubic_structure, 1, 1, 0) == pytest.approx(
            lattice_param / math.sqrt(2), abs=0.01
        )
        assert surfaces.d_spacing(simple_cubic_structure, 1, 1, 1) == pytest.approx(
            lattice_param / math.sqrt(3), abs=0.01
        )

    def test_d_200_is_half_lattice_param(self, simple_cubic_structure: dict) -> None:
        """d_200 = a/2."""
        assert surfaces.d_spacing(simple_cubic_structure, 2, 0, 0) == pytest.approx(
            2.0, abs=0.01
        )
    ...
    def test_d_hkl_general_pymatgen(self) -> None:
        ...
        assert surfaces.d_spacing(struct, *hkl) == pytest.approx(expected, abs=0.01)
...
class TestSurfaceEnergy:
    ...
    def test_formula(self) -> None:
        ...
        energy = surfaces.calculate_energy(
...
class TestMillerIndexEdgeCases:
    ...
    def test_enumerate_miller(
        ...
        indices = surfaces.enumerate_miller(max_idx)
    ...
    def test_d_spacing_zero_miller_raises(self, simple_cubic_structure: dict) -> None:
        ...
        with pytest.raises(ValueError):
            surfaces.d_spacing(simple_cubic_structure, 0, 0, 0)
    ...
    def test_d_spacing_equivalent_miller(self, simple_cubic_structure: dict) -> None:
        ...
        d_100 = surfaces.d_spacing(simple_cubic_structure, 1, 0, 0)
        d_200 = surfaces.d_spacing(simple_cubic_structure, 2, 0, 0)
