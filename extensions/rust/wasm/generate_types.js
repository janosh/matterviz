...
function generate() {
  ...
  // (regexes rely on semicolons being present)
  // 1. InitOutput interface (internal low-level wasm bindings with `any` types)
  patched = patched.replace(/\nexport interface InitOutput \{[\s\S]*?\n\}\n/, `\n`)
  // 2. SyncInitInput type and initSync function + its JSDoc
  patched = patched.replace(/\nexport type SyncInitInput[^\n]*\n/, `\n`)
  patched = patched.replace(
    /\n\/\*\*\n \* Instantiates[\s\S]*?\*\/\nexport function initSync[^\n]*\n/,
    `\n`,
  )
  // 3. __wbg_init default export + its JSDoc (we provide our own init)
  patched = patched.replace(
