...
- Settings: `matterviz.{histogram,bar,box,scatter}.display.*` and `matterviz.plot.show_{x,y}_zero_line` are replaced by one `matterviz.plot.display.*` group; `matterviz.structure.show_gizmo` becomes `matterviz.structure.gizmo: boolean | GizmoOptions`; `composition.{display_mode,color_scheme}` and `plot.{x,y}_scale_type` are removed https://github.com/janosh/matterviz/pull/438; `matterviz.trajectory.show_parsing_progress` is removed too: it was generated into the VS Code configuration but never read (the webview mounts `Trajectory` after the host has parsed, and `TrajectoryFileViewer` has no such prop) https://github.com/janosh/matterviz/pull/439
