...
- Removed exports: `Violin` (use `<BoxPlot kind="violin">`), `element_groups`/`element_group_keys`/`ElementGroup`, `PeriodicTableControls`, `SubpageGrid`, `push_edit`/`step_history`, `AnalysisPaneContext`, `with_hover_pointer`, `get_nice_data_range`, `get_projection_source_entries`, and the types `MsdPositions` (use `TrajectoryPositionStream`), `TrajectoryFormat`, `TrajectoryInspectorTab`, `ScalarGridLike`, `InitialRanges`, `TickConfig`, `BinnedPointLabelPlacementConfig`, `BinnedPointTooltipPayload`
- `is_editable_target` → `is_editable_event_target` and `fuzzy_match` → `svelte-widgets/utils` (flipped argument order `fuzzy_match(query, text)`)
- `AxisConfig.tick: TickConfig` → `AxisConfig.tick_label: TickLabelConfig`; `Label.format_type` → `datetime_format`; `PlotControls`' `children`/`post_children` take no `ControlsState`
- Formula formatters take an options object (`get_alphabetical_formula(input, { plain_text, delim, amount_format })` etc.) and default to `AMOUNT_FORMAT = '.3~f'`, was `.3~s` which wrote `C1000` as `C1k`
- `bonding_strategies.*` → named exports on `matterviz/structure`; `ReferenceLine3D`/`ReferencePlane` → `plot/scatter-3d`
- Loading: `create_data_url_loader`/`create_viewer_loader`/`open_structure_text` → `open_material`/`create_material_loader`/`apply_structure_material`; `ParseResult` is a union discriminated on `type`
- Theming is `data-theme` + `color-scheme` on the root and nothing else: the `matterviz/theme/themes` subpath, `get_theme_css`, `resolve_theme_mode`, `theme_state.system_mode` and the `MATTERVIZ_THEMES`/`MATTERVIZ_CSS_MAP` globals are gone
- Viewer shortcuts are plain letters, not chords: `f` for fullscreen everywhere, `i`/`g` in `Structure`, `e` for export panes
- Bond detection: `electroneg_ratio`'s `same_species_penalty` and `SHELL_TOL` are replaced by `cation_cation_penalty` (default 0), so metals and polyanion frameworks get different coordination numbers

### ⚠️ Breaking Changes

- Public API cut: 20 removed exports, the renames above, and ~60 never-set props dropped across `PeriodicTable`, `HeatmapTable`, `TrajectoryInfoPane`, `ChemPotDiagram2D/3D`, `SymmetryElements`, `VolumeSlice`, `FullscreenButton`, `Hull3DProps`, `HierarchyShell` and `LegendConfig` https://github.com/janosh/matterviz/pull/447
- `electroneg_ratio` classifies every pair as metallic (both ends metals, expected length from the new `metallic_radius`, within the first metal-metal shell of either end), lenient (one end a terminal anion former, covalent radii with a soft shell penalty) or tight (both ends non-terminal); `same_species_penalty`, `SHELL_TOL`, the `PairReach` replay and the per-site radii array are gone, and cation-cation contacts around a saturated center are scaled by `cation_cation_penalty` instead https://github.com/janosh/matterviz/pull/456
- Every theme token is defined once in `src/lib/app.css` as `light-dark(light, dark)` on `:root`/`:host`, so the runtime writes two attributes instead of 40 inline CSS variables and `theme/themes.mjs` (282 lines), its `static/themes.mjs` symlink and the theme globals are deleted; `is_dark_mode(element?)`/`watch_dark_mode(cb, element?)` resolve the nearest declared `color-scheme` at or above the element (crossing shadow roots, OS preference as fallback) rather than re-deriving from `data-theme` + `localStorage` https://github.com/janosh/matterviz/pull/445
- `FullscreenButton` owns `f` for all 8 fullscreen-capable viewers, replacing five per-viewer handlers and the `Cmd/Ctrl+F` bindings that shadowed find-in-page; `Structure` takes `i`/`g`, export panes `e`; a `data-mv-fullscreen-root` marker makes a nested viewer defer to the outer one, and undo/redo, copy, the command palette and `Cmd+Arrow` seek stay chords https://github.com/janosh/matterviz/pull/454
- `phonon_mode_trajectory` splits into `phonon_supercell` → `phonon_mode_pattern` (packed `Float64Array` complex displacements) → `phonon_mode_run` (frames synthesised on read), keeping the old name as the wrapper; `spectral/complex-mode.ts` and its exports are gone, the frame guard becomes `MAX_PHONON_SUPERCELL_SITES = 200_000` and `DEFAULT_PHONON_FPS` goes 12 → 24 https://github.com/janosh/matterviz/pull/443
- Viewer loading collapses into `open_material()` + `create_material_loader`: `io/data-url.ts` and `scene/viewer-loader.svelte.ts` are removed, `open_structure_text` becomes `apply_structure_material`, and `ParseResult` is discriminated on `type` (`trajectory`/`structure`/`fermi_surface`/`isosurface`/`convex_hull`/`phase_diagram`/`json_browser`/`vaspout_electronic`) with `WireParseResult` covering the worker boundary
- Hierarchy `min_fraction` measures each node against the view's zoom root rather than the data root, so drilling into a parent re-measures its children and dissolves the bucket; chart export filenames come from the frame's resolved axis labels https://github.com/janosh/matterviz/pull/455

### 🚀 New Features

- `matterviz/slab` builds surface slabs from a bulk `Crystal`: `make_oriented_bulk` re-expresses the cell so `c` crosses the (hkl) planes exactly once (exact integer lattice arithmetic), `enumerate_terminations` groups sites into layers along the surface normal and drops translation-equivalent terminations, `make_slab` cleaves, stacks whole repeats to `min_slab_thickness` and opens vacuum (`Slab = Crystal & { slab_info }`), and `SlabBuilder.svelte` wraps the controls around a `Structure` viewer; demo at `/structure/slab` https://github.com/janosh/matterviz/pull/456
- `TernaryPlot` + `TernaryControls` (`matterviz/plot`): barycentric triangle with constant-fraction grid lines, edge ticks, marker and line series, per-point color values and a color bar, on shared `ternary.ts` geometry (`ternary_to_xy`, `xy_to_ternary`, `ternary_grid_lines`, `ternary_layout`) that the isothermal ternary section and `chempot-diagram/compute.ts` now use instead of their own copies; demo at `/plot/ternary` https://github.com/janosh/matterviz/pull/456
- VASP trajectories: `parse_vasp_outcar` reads one frame per POSITION/TOTAL-FORCE table with the surrounding cell, energy, stress and thermostat lines, and `parse_vasprun_xml` scans `<calculation>` blocks by text (workers have no `DOMParser`, and a multi-hundred-MB vasprun would not fit one anyway); `VASP_TRAJECTORY_FILES`/`VASPRUN_REGEX` replace `XDATCAR_REGEX` throughout format detection, auto-render eligibility, file-type paints and the VS Code / JupyterLab editor selectors, so `OUTCAR`, `OUTCAR.gz` and `vasprun.xml` open in the viewer https://github.com/janosh/matterviz/pull/456
- Synthesis planner at `matterviz/synthesis-planning`: `plan_synthesis` enumerates routes by LP, with `balance_reaction`, `analyze_selectivity`, `onset_temperature`, `build_recipe`, `score_route`, `PRECURSOR_LIBRARY`, the `SynthesisPlanner`/`RouteTable`/`RecipeCard`/`ReactionSlicePlot` components and an LLM tool surface (`SYNTHESIS_PLANNER_TOOL`); backed by a new `math.solve_linear_program` (two-phase tableau simplex on Bland's rule) and a `components` prop on `ConvexHull` keying entries by pseudo-components (`{ BaO: 0.4, TiO2: 0.6 }`); demo at `/convex-hull/synthesis-planning` https://github.com/janosh/matterviz/pull/450
- `open_material(source, options)` routes URL, `File` and in-memory sources through one acquire/parse path with `signal`, `on_progress`, `on_acquired` (so an HDF5 group re-pick doesn't re-download), a readonly `MaterialProvenance`, `dispose()` and `MaterialOpenError { stage, provenance }`; `create_material_loader`, `create_drop_batch_handler` and the `raw_file_drop_zone` attachment back `Structure`, `TrajectoryFileViewer`, `BrillouinZone`, `FermiSurface` and `ConvexHull`
- Trajectory RDF pane: `collect_trajectory_rdf` samples frames evenly, normalises each by its own cell volume and averages every element pair's g(r) in a worker (`DEFAULT_RDF_MAX_FRAMES = 100`, cutoff 10 Å, 200 bins, mixed-occupancy sites feeding every partial), `rdf_shell` reads first peak, first minimum and `CN(B|A)`/`CN(A|B)`, and MSD, VACF/VDOS and g(r) curves download as CSV https://github.com/janosh/matterviz/pull/443
- Chart export from the controls pane: `export_chart_image`/`create_chart_exporter`/`to_csv` plus `on_export`/`export_formats` on `PlotControls` give `ScatterPlot`, `BarPlot`, `Histogram` and `BoxPlot` PNG, SVG and CSV — one row per point for scatter and bar, bin edges for histogram, a five-number summary for box plots https://github.com/janosh/matterviz/pull/455
- Per-point error bars via `DataSeries.x_error`/`y_error`, taking the same scalar, per-point array or `{ upper, lower }` shape as `ErrorBand.error`, with axes widened to include the caps and one SVG path per series; `Alt`+drag rect-selects points (shift pans, plain drag zooms), reported through `on_select` and bindable `selected_points` https://github.com/janosh/matterviz/pull/455
- Keyboard access to plot marks: `create_roving_focus` gives each Cartesian chart one tab stop with arrows/Home/End moving between scatter points, bars, line points, bins and boxes, where every mark used to be its own tab stop (up to 10k presses); canvas-rendered scatters past `CANVAS_MARKER_THRESHOLD` get an arrow-key cursor announced in a polite live region, and every visible hierarchy arc is focusable and labelled https://github.com/janosh/matterviz/pull/455
- Bucketing of small items: hierarchy `max_children` keeps the N largest children per parent and buckets the rest, `other_label` accepts a callback so an arc can say `312 smaller jobs`, and `Sankey` gains `min_fraction`/`max_links` folding a node's small outgoing links into one 'Other' link (only links to terminal targets, so no downstream flow is deleted) https://github.com/janosh/matterviz/pull/455
- `metallic_radius` (12-coordinate, 68 metals from pymatgen) is an optional field on `ChemicalElement`, and the exported `expected_bond_length(elem_a, elem_b)` is shared with `find_image_atoms` so periodic image completion reaches exactly as far as the bond detector https://github.com/janosh/matterviz/pull/456
- IFermi `FermiSurface` JSON (`"@class": "FermiSurface"` with a band-keyed `isosurfaces` object) is recognized by `detect_view_type`, so it gets typed meshes instead of the JSON browser fallback
- `ScatterPlot` exposes bindable `resolved_padding` and `shared_resolved_padding_floor()` feeds both panels' padding through a monotonic floor, so `BandsAndDos`/`BrillouinBandsDos` keep the shared energy axis aligned without oscillating https://github.com/janosh/matterviz/pull/445

### 🐛 Bug Fixes

- TorchSim writes an all-zero cell with `pbc=[false,false,false]` for molecules, so every frame failed as a torn write (`cell volume is zero`) and those trajectories would not open at all — a cell is now required only when some axis is periodic, and degenerate cells on non-periodic runs are dropped so the run parses lattice-free like XYZ https://github.com/janosh/matterviz/pull/448
- Opening any file in a VS Code webview hung ~20 s: the blob-module shim let `new Worker()` succeed, but its inner `import()` of the vscode-cdn URL runs in worker context where the service worker does not intercept, so Chromium resolved the fake host over DNS before falling back — `install_cross_origin_worker_guard` now throws `SecurityError` synchronously, which every worker client already handles https://github.com/janosh/matterviz/pull/450
- Coordination numbers from `electroneg_ratio`: P in LiFePO4 bonded to 3 Fe/Li neighbours through the PO4 oxygens (CN 7 → 4), Al in the LuAl2 Laves phase lost its 6 Al-Al contacts (CN 6 → 12), bcc Cs was stretched beyond covalent reach (CN 0 → 8) and high-entropy alloys spread over CN 4-13 instead of peaking at 8; bcc Fe/Cs, LuAl2, Cu2O, LiFePO4 and a Pa-3 pyrite fixture join the coordination benchmark https://github.com/janosh/matterviz/pull/456
- `compute_polyhedra` dropped bonded anions farther than 1.3 × the shortest bond, so tetragonal BaTiO3 drew Ti as a 5-vertex square pyramid next to 6 Ti-O bonds; the polyhedron now has exactly the vertices the detector bonded (default `polyhedra_opacity` 0.2 → 0.25 so faces read against the bond sticks) https://github.com/janosh/matterviz/pull/456
- `filter_entries_at_temperature` wrote the per-atom G(T) into `PhaseData.energy`, which is a total, skewing every downstream formation energy; also fixed: phase-diagram export serializing the real `<svg>` node, chempot projection axes derived from all entries instead of the temperature slice, `nice_range` leaving an inverted log domain, `Math.max(...values)` blowing the argument limit on 1e7-point grids, CIF `_atom_site_type_symbol` values like `Fe2+`/`NO3`, and `FormulaFilter` crashing on an invalid committed wildcard formula https://github.com/janosh/matterviz/pull/447
- Trajectories up to 400 MB stay openable in VS Code webviews: the 25 MiB main-thread fallback cap is skipped for indexable files (multi-frame XYZ, ASE `.traj`) located frame by frame without decoding; `PositionAccumulator` records each site's `id` from frame 0 and throws naming the frame and site when a later frame carries a different one, so a GCMC frame that swapped atoms without changing the count no longer pairs unrelated atoms in MSD, VACF and trails https://github.com/janosh/matterviz/pull/450
- VS Code webviews flashed white and rendered the trajectory scrub bar light over dark themes: `create_html` writes `color-scheme` (mapped through a `Record` over `ThemeName`, so a new theme is a type error) and `background-color` inline before the CSS link, and `build.cssTarget` comes from the shared `make_config().build` so LightningCSS keeps native `light-dark()` (212 native calls, no `prefers-color-scheme` polyfill, against 0 and 1 before) https://github.com/janosh/matterviz/pull/454
- Twelve keydown handlers claimed modifier chords and then `preventDefault`'d them away from the browser (the convex hull's guard sat after the Escape/Enter branches, so `Cmd/Ctrl+Enter` selected the hovered entry) — the guard now runs before every branch; toggles also guard `event.repeat`, since a held key autorepeated `Structure`'s `i`/`g`/`a`/`e` and `HeatmapMatrix`'s `e` (one file download per repeat), while arrows and temperature stepping still repeat https://github.com/janosh/matterviz/pull/454
- A clicked viewer is both focused and hovered, so `Structure`'s root and window keydown passes both ran and each toggle cancelled itself out — `g` did nothing at all once the viewer had been clicked, and the existing test missed it by calling `.focus()`, which never moves the pointer https://github.com/janosh/matterviz/pull/454
- LAMMPS element resolution is one chain — `atom_type_mapping`, then the `element` column, then atomic number — so a dump carrying both reads the element column, and one whose element column holds type labels (`Type1`) guesses and warns instead of throwing; the trajectory session's `current_structure` is a `$derived`, so a swapped run fits the camera to the new run instead of the previous one https://github.com/janosh/matterviz/pull/443
- anywidget standalone bundle: `resolve.dedupe: ['three']` stops two copies of `three/webgpu` shipping, whose `LightsNode` reported "Light node not found" and rendered every atom black outside the notebook it was built for (pymatviz `to_html`, report exports); the theme block is emitted after `app.css` so its dark `:root` fallbacks no longer override the detected theme https://github.com/janosh/matterviz/pull/443
- Sankey folding attached fills and tooltips to the wrong records (metadata indexed the original arrays while the indices came from the folded graph), silently merged two nodes sharing a label, folded links whose target resolved to nothing, and could mint a bucket id colliding with a real node called `src/Other` https://github.com/janosh/matterviz/pull/455
- Hierarchy ids and treemap zoom: an unlabeled node's id fell back to its index in the live `children` array that `sort` and bucketing both rewrite, so a persisted `zoom_root_id` resolved to a _different_ node — sibling indices are captured before anything reorders; `align_tiling` re-expresses the previous tiling in the new index space matched by arc id so a dissolved bucket no longer snaps the animation https://github.com/janosh/matterviz/pull/455
- A `<canvas>` inside a `<foreignObject>` is a live bitmap the serializer emits as an empty element, so exported PNG/SVG lost canvas-rendered points entirely (each is now swapped for an `<image>` via `inline_foreign_canvases`); `error_length` reported only `upper`, so a short `lower` read as zero, and x error bars used the raw x scale while y used the log-floored one, dropping the whole bar when an x error reached zero on a log axis https://github.com/janosh/matterviz/pull/455
- An attribute set before `{...rest}` is replaced by the spread rather than merged, so any caller passing `class`/`style` silently dropped the component's own styling — fixed in 13 components, and the same bug kept `Treemap`'s contrasting chrome color out of the DOM (icons stayed dark over a dark cell), now an explicit `chrome_color` prop on `ChartShell` https://github.com/janosh/matterviz/pull/455
- Theme detection: marker words match case-insensitively (JupyterLab's `data-theme` is title-cased), the `only` modifier is skipped when reading `color-scheme: only dark`, and `watch_dark_mode` stays silent on subscribe (a synchronous first call hit `effect_update_depth_exceeded` and rendered no viewers) https://github.com/janosh/matterviz/pull/445
- `SequenceControls`' `.step-labels` box was 0 px tall and its `overflow: hidden` clipped every tick and label on the deployed site; `SpacegroupBarPlot` and `Trajectory` spread caller padding after their own reservations (both take `max(caller, reserved)` now); `ColorBar` tick thinning keeps the first and last tick instead of striding from the start and dropping the upper range label; the element detail pages were broken in dev https://github.com/janosh/matterviz/pull/445
- `PlotLegend` hand-drew five shapes and rendered nothing for the other symbol types; swatches now draw the same `symbol_map` d3 path the plot uses on a `-7..7` viewBox, since star and diamond at `size(50)` reach ±6.7 and were clipped by the old `-5..5` box https://github.com/janosh/matterviz/pull/456
- `ternary_grid_lines` throws for `grid_step` below 0.01 instead of allocating billions of line objects (1e-9) or looping without end (`Number.MIN_VALUE`); `TernaryPlot` reads its color range through `array_extent` (a `Math.min`/`max` spread throws past ~125k points) and its legend line swatch takes `line_style.color` like the rendered path; the trajectory parse-warning and frame-error banners sat at the same `z-index: 2` as the bottom-anchored `AtomLegend`, which painted over their text https://github.com/janosh/matterviz/pull/456
- `is_trajectory_file` no longer rejects multi-frame XYZ content behind a name that gives no extension hint (`blob:` URLs, extensionless endpoints) — frame counting decides when the filename is silent, and an explicit `.xyz`/`.extxyz` name still requires ≥ 2 frames

### 🛠 Enhancements

- XYZ/LAMMPS parsing: `LineScanner` decodes plain decimals (≤ 15 significant digits) straight off the char codes in one allocation-free pass, rounding bit-identically to `Number()` and deferring exponents and Fortran `D` to `parse_float_token`, replacing `trim().split(/\s+/)` + `Number()` per column; `iter_xyz_frames`/`index_xyz_frames` walk the untouched text by character offset, so a 10M-line dump no longer allocates one string per line, and `has_multiple_xyz_frames` sniffs a 64 KB head before the 8 MB one https://github.com/janosh/matterviz/pull/450
- Per-frame viewer overhead: `StructureScene` makes one pass over `atom_data` where it made seven and hoists prop reads out of loops that walked two spread-prop proxy layers, `get_explicit_bond_metadata`/`is_vector_key`/`get_structure_vector_keys` are memoised, `topology_signature` short-circuits on shared label/species arrays, `electroneg_ratio` resolves element data once per element rather than per site, and the session frame cache is a plain `Map` (a `SvelteMap` minted a signal per frame index) https://github.com/janosh/matterviz/pull/443
- Trajectories accept frames with differing atom counts — in-memory runs validate only frame 0, and `collect_positions` remains the one place a constant count is required — and the LAMMPS parser keeps frames whose atom ID set changed instead of dropping them https://github.com/janosh/matterviz/pull/450
- Phone-width and touch layouts, audited across every demo route at 360/390/768 px and under touch emulation: hover-revealed chrome shows on `(hover: none)`, controls are 32 px on `(pointer: coarse)` with a 24 px touch strip on range inputs, panes cap to the space left inside the viewer (JS-measured, since `cqh` resolves to 0 when the height comes from `min-height`), 3D canvases use `touch-action: pan-y` so a vertical swipe scrolls while a horizontal drag orbits, and the periodic table gets a 2 rem tile floor with tap-to-preview https://github.com/janosh/matterviz/pull/445
- Plot chrome placement: a legend wider than 45 % of the plot moves to a strip below it with auto tracks in the orientation the solver counted them in, a colorbar wider than 70 % moves to the top margin, outside legends cap at half the plot height and scroll, and rotated y labels trade anchors at the ends like upright x labels https://github.com/janosh/matterviz/pull/445
- `TrajectoryInfoPane` renders one card per varying property — current value, range, mean ± σ and least-squares drift fitted against the recorded `step`, capped at 8 sections so a 30-column LAMMPS log stays readable — from the new `summarize_properties`; `PhononModeExplorer` derives supercell, pattern and run separately so an amplitude drag rebuilds only the run and the camera keeps its orbit https://github.com/janosh/matterviz/pull/443
- Chempot display toggles no longer re-run the worker, `JsonTree` pages children by 100, `FullscreenButton` reports transitions from the `fullscreenchange` event so a browser-rejected request never reaches the host, and `parse_in_worker` gains `owns_content` to transfer rather than clone an oversized `ArrayBuffer` the caller will not read again https://github.com/janosh/matterviz/pull/447
- Tooltip placement scores the pointer glyph as one more exclusion rect (`avoid_cursor`, default true), so a tooltip too wide to sit beside its anchor stops falling below and putting the cursor on its first line; charts that snap the anchor to a mark opt out https://github.com/janosh/matterviz/pull/455
- `electroneg_ratio` caches the per-candidate pair index, normalized distance and metallic flag in pass 1 so passes 2-4 stop recomputing them (bit-identical results over the site structures and the CN benchmark), and `pbc.ts` hoists the element lookup out of its inner loop https://github.com/janosh/matterviz/pull/456
- Dependency bumps: vite-plus 0.3.0 (bundling vitest 4.1.11), DOMPurify 3.4.7 → 3.4.14, `@spglib/moyo-wasm` ^0.16.0, `@jupyterlab/*` ^4.6, happy-dom ^20.11.6; the vitest setup dispatches happy-dom's base-class `nodeName` getter to the subclass getter, since DOMPurify ≥ 3.4.13 reads it clobber-safe through `Node.prototype` and happy-dom returned `''`, so under test the sanitizer stripped all markup https://github.com/janosh/matterviz/pull/445
- Docs site: the nav collapses from 11 per-directory entries into six topical dropdowns driven by a `NAV_GROUPS` table that throws at import for a route no group claims, the redundant `/how-to` index page is gone, and every card on `/plot` has its own icon instead of six sharing a generic one https://github.com/janosh/matterviz/pull/456

### 💡 Refactoring

- Repo-wide dedupe across 467 files (+8440/−10247): one cell pipeline shared by the structure parsers with lazy frac-coord conversion on export, one place computing trajectory frame `volume`/`time_step`, single implementations of Bands/DOS axis sync, hull colours, legend markup, volumetric sampling and mesh building, one ASE decoder, and tests folded into `it.each` tables across ~25 files https://github.com/janosh/matterviz/pull/447
- svelte-widgets 1.6.1 absorbs hand-rolled site and library code: the docs source-link vite plugin and runtime (~170 lines replaced by 3 lines of wiring), the Pagefind loader and local `SubpageGrid` (→ `PageSearch` and upstream `SubpageGrid`), and `fuzzy_match`/`is_editable_event_target`/`storage_*`/`persisted_choice`/`ToastStore` behind `FormulaFilter` history, `viewer-state` storage, theme preference and the `Structure` edit toast; `remark-math`/`rehype-katex` and the KaTeX CSS link are deleted since no page uses math https://github.com/janosh/matterviz/pull/448
- `trajectory/analysis.ts` gains the shared `sweep_frame_plan`/`sweep_frames` used by both the structure-id and RDF panes, `analysis_fail`/`lag_axis_label` are shared by `calc-msd`/`calc-vacf`/`positions`, bond-key helpers and the `site_base_radius` precedence rule are shared by `StructureScene`/`bonding.ts`/`camera-fit.ts`/`AtomLegend`, and `watch_theme` becomes a per-widget closure instead of a refcounted module singleton, with nine components' duplicated `@media (hover: none)` reveals collapsing into one global `.hover-visible` rule
...
- `TrajectoryRun.signals` / `TrajectoryRunSummary.signals` / `LazyTrajectorySource.signals` absorb `signal_descriptors`: one `signals: Record<string, TrajectorySignal | TrajectorySignalDescriptor>` (`TrajectoryRunSignal`) whose entries are loaded (`{ values: Float64Array, sample_shape, steps, unit? }`) or lazy descriptors (`{ sample_shape, sample_count, frame_aligned?, unit? }`, streamed by `collect_positions({ signal_keys })`; `frame_aligned` is set by the HDF5 parsers from the signal's step axis vs the geometry steps and is the only thing that routes a `[n_atoms, 3]` descriptor through `vector_keys`, since `sample_count === frame_count` alone does not make the axes match), told apart by `'values' in signal` via `is_loaded_signal`/`is_signal_descriptor`. Wire format of the run summary crossing the parse-worker port and the VS Code `large_file_response.run_summary`: before `{ signals?, signal_descriptors? }`, after `{ signals? }` (a lazy HDF5 run that sent `signal_descriptors: { velocity: { sample_shape: [n, 3], sample_count: 4, unit: 'A/fs' } }` sends the same object under `signals`); `ParsedTrajectory.signal_descriptors` is removed
...
- Installable from git: `npm install github:janosh/matterviz#main` yields a usable package (see Packaging above); new root script `pnpm build:anywidget` chains `package:dist` → anywidget install → bundle build (documented in `extensions/anywidget/readme.md`)
...
- Perf regression tripwires: `tests/vitest/perf-baselines.test.ts` times seeded synthetic workloads for `ScatterPlot` (100k canvas points), `Trajectory` (mount + 100 frame switches), `JsonTree`, `HeatmapTable`, `calculate_e_above_hull`, CHGCAR parse, marching cubes, `neighbor_query`, XRD, polyhedra 8000-site detect/merge, 4D quickhull, 1M-point density binning, the flyaway-atom `neighbor_query` grid, coordination colours and 27k-site `make_supercell` against stored medians with a 2× band scaled by a reference workload, in its own CI job (`MATTERVIZ_PERF=1` locally); the default vitest run has no wall-clock assertions left; the Playwright trajectory perf spec builds its run in-page instead of skipping on CI; new Playwright smoke specs for `FermiSurface` (`tests/playwright/fermi-surface.test.ts`) and `IsobaricTernaryPhaseDiagram` (`tests/playwright/phase-diagram/isobaric-ternary.test.ts`) https://github.com/janosh/matterviz/pull/439
