...
- Spectral: `BaseBandStructure.recip_lattice` and `PhononModeData.reciprocal_lattice` are removed; XRD parsers throw on malformed input instead of falling back to `start=0`/`step=0.02` https://github.com/janosh/matterviz/pull/438
...
- Add `BinnedScatterPlot` `pan`, `InfoPaneCards` `page_size`/`card_attrs`/`row_value`, `MillerIndexInput` bar notation, `ChemPotTooltip`, `is_planar` on chemical-potential domains and `volume_from_json` https://github.com/janosh/matterviz/pull/438
