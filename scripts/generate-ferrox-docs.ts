...
async function postprocess_markdown(filepath: string): Promise<void> {
  try {
    ...
    // Add labels to empty anchor links generated by doc tools to satisfy Svelte a11y checks.
    // Example: <a id="ferrox.xrd"></a> -> <a id="ferrox.xrd" aria-label="ferrox.xrd"></a>
    content = content.replace(
      /<a id="([^"]+)"><\/a>/g,
      (_match, anchor_id) => `<a id="${anchor_id}" aria-label="${anchor_id}"></a>`,
    )
